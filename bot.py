...
# GOOGLE reCAPTCHA KEYS (senin verdiğin v2 key'ler)
SITE_KEY = "6LfNH9srAAAAADykYLJ0UujXd2AsYEUTDcZ-LegN"       # HTML'de görünür
SECRET_KEY = "6LfNH9srAAAAAHDqEC81oq4vCQTqRl0bDUriU2HN"   # backend doğrulama için GİZLİ
...
def rate_limit_and_verify():
    # verify ve static dosyaları sınırlamadan geç
...
@app.route('/')
def index():
    # index3.html templates içinde olmalı
...
def verify():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = "Doğrulama sırasında hata oluştu (istek hatası)."
            # opsiyonel: logla: print("recaptcha error:", e)
            return render_template('robot_dogrulama.html', site_key=SITE_KEY, error=error)

        # v2 response: result['success'] True/False
        if result.get("success"):
            session['verified'] = True
            return redirect('/')
        else:
            # Google'ın döndürdüğü hatayı görmek istersen result içeriğini logla
